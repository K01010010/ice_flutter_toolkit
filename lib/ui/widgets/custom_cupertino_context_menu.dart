...
// was eyeballed by overlapping the CustomCupertinoContextMenu with a context menu
...
const Duration _kModalPopupTransitionDuration = Duration(milliseconds: 1);

// The duration it takes for the CustomCupertinoContextMenu to open.
// This value was eyeballed from the XCode simulator running iOS 16.0.
const Duration _previewLongPressTimeout = Duration(milliseconds: 1);
...
/// default child and the preview when the CustomCupertinoContextMenu is open.
typedef CustomCupertinoContextMenuBuilder = Widget Function(
  BuildContext context,
  Animation<double> animation,
);
...
/// unconstrained.
///
/// When closed, the [CustomCupertinoContextMenu] displays the child as if the
...
///
/// {@tool dartpad}
/// This sample shows a very simple [CustomCupertinoContextMenu] for the Flutter logo.
...
///
/// {@tool dartpad}
/// This sample shows a similar CustomCupertinoContextMenu, this time using [builder]
...
class CustomCupertinoContextMenu extends StatefulWidget {
  ...
  ///
  /// [builder] is required.
  CustomCupertinoContextMenu.builder({
    super.key,
    required this.actions,
  ...
  ///       end: BorderRadius.circular(CustomCupertinoContextMenu.kOpenBorderRadius),
  ///     ).animate(
  ///       CurvedAnimation(
  ///         parent: animation,
  ///         curve: Interval(
  ///           CustomCupertinoContextMenu.animationOpensAt,
  ///           1.0,
  ///         ),
  ///       ),
  ///     );
  ...
  ///        color: Color(0xFFFFFFFF),
  ///        boxShadow: CustomCupertinoContextMenu.kEndBoxShadow,
  ///       ),
  ///      ).animate(
  ///        CurvedAnimation(
  ///         parent: animation,
  ...
  ///     return Container(
  ///       decoration:
  ///         animation.value < CustomCupertinoContextMenu.animationOpensAt ? boxDecorationAnimation.value : null,
  ///       child: FittedBox(
  ///         fit: BoxFit.cover,
  ///         child: ClipRRect(
  ...
  /// The point at which the CustomCupertinoContextMenu begins to animate
  ...
  /// [CustomCupertinoContextMenu] is closed, when it is in the middle of opening,
  ...
  /// passed to the builder, using values exposed by [CustomCupertinoContextMenu],
  /// like [CustomCupertinoContextMenu.kEndBoxShadow], to match the native iOS
  /// animation as close as desired.
  ///
  /// ** See code in examples/api/lib/cupertino/context_menu/cupertino_context_menu.1.dart **
  /// {@end-tool}
  final CustomCupertinoContextMenuBuilder builder;
  ...
  ///
  /// These actions are typically [CustomCupertinoContextMenuAction]s.
  ///
  ...
  /// If true, clicking on the [CustomCupertinoContextMenuAction]s will
      ...
      builder: (BuildContext context, Animation<double> animation) {
        if (widget.child == null) {
          final Animation<double> localAnimation = Tween<double>(
                  begin: CustomCupertinoContextMenu.animationOpensAt, end: 1)
              .animate(animation);
...
class _ContextMenuRoute<T> extends PopupRoute<T> {
  ...
  _ContextMenuRoute({
    ...
    this.barrierLabel,
    this.menuPadding,
    CustomCupertinoContextMenuBuilder? builder,
    super.filter,
  ...
  final CustomCupertinoContextMenuBuilder? _builder;
  final GlobalKey _childGlobalKey = GlobalKey();
...
class _ContextMenuRouteStaticState extends State<_ContextMenuRouteStatic>
  ...
  // The CustomCupertinoContextMenuSheet disappears at this scale.
  ...
  List<Widget> _getChildren(
    ...
    final Widget simpleSheet = AnimatedBuilder(
    ...
    final Widget simpleChild = Align(
      alignment: _getChildAlignment(
        widget.orientation,
    ...
    final Expanded expChild = Expanded(child: simpleChild);
    final Expanded expSheet = Expanded(child: simpleSheet);
    ...
    switch (contextMenuLocation) {
      case _ContextMenuLocation.center:
        return <Widget>[expChild, spacer, expSheet];
      case _ContextMenuLocation.right:
        return orientation == Orientation.portrait
            ? <Widget>[expChild, spacer, expSheet]
            : <Widget>[expSheet, spacer, expChild];
      case _ContextMenuLocation.left:
        return <Widget>[expChild, spacer, expSheet];
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.child.key != null &&
        widget.child.key.runtimeType == GlobalObjectKey) {
      var result = widget.child.key as GlobalObjectKey; //<State<StatefulWidget>>;

      RenderBox? box = (result.currentContext?.findRenderObject() as RenderBox?);
    ...
    final List<Widget> children = _getChildren(
        widget.orientation, widget.contextMenuLocation,
        position: position, screenSize: screenSize);
...
// list of actions that are typically CustomCupertinoContextMenuActions.
